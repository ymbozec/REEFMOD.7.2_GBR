...
% Yves-Marie Bozec, y.bozec@uq.edu.au, (Mar 2024 - Aug 2025)
%
% This is an incremental version of 7.0 which was used for simulating the RRAP FY2024 counterfactuals (April 2024)
% and version 7.1 (first attempt at implementing wave-energy based cyclone refugia - March 2024).
% Version 7.2 was used for simulating the RRAP FY2025 counterfactuals (Feb 2025).
%
...
% Note there is an error with default control strategy (GBR-wide) as the ecological threshold is set to 0 due to 
% code line 111 in f_makeReefList_TS that is deactivated (commented) for unknown reason, with implications for culling
...
% - revised initial conditions of coral cover using AIMS LTMP+MMP, this time extracted from the RimRep Data Management
% System -> GBR_AIMS_OBS_CORAL_COVER_2024.mat (now used for initialisation and validation of hindcast)
% - CoTS observations from manta tows of the Control Program (but not AIMS LTMP) was extended to end of 2024
%
% Code for restoration intervention dating back to 2022 has been tidied up. 
% Outplants and native corals allowed to breed. Thermal enhancement for outplants allowed with specific mean, var and max HT
% Revised code for forcing external larvae in the absence of connectivity.
% Revised code for forcing connectivity for regional (not GBR-wide) simulations
% Added control on the cooling effect of cyclones on bleaching (when both occur at the same time step):
% -> set to 0 (no cooling) for the hindcast but 1 (no bleaching) in the forecast
%
%__________________________________________________________________________
clear
...
record_spawning_chronology_GBR4 = zeros(NB_SIMULATIONS,META.nb_time_steps,'uint8'); % sequence of GBR4 connectivity matrices (CoTS)
record_spawning_chronology_GBR1 = zeros(NB_SIMULATIONS,META.nb_time_steps,'uint8'); % sequence of GBR1 connectivity matrices (corals)
record_spawning_event_coral_GBR1 = zeros(NB_SIMULATIONS,META.nb_time_steps,'uint8'); % spawning event selected every year (coral)
...
for simul = 1:NB_SIMULATIONS
    ...
    record_spawning_event_coral_GBR1(simul,:) =  squeeze(OUTPUTS(simul).RECORD.spawning_event_coral_GBR1);
